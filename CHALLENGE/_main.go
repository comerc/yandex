...
func (m *MockStore) Get(ctx context.Context, id int64) (*Item, error) {
	...
	// если отменится контекст, функция выйдет не выполнив основное тело - поясни
	select {
	case <-time.After(m.delay):
	case <-ctx.Done():
		// fmt.Printf("%s: request cancelled\n", m.name)
...
func (e *Example) Get(ctx context.Context, id int64) (*Item, error) {
	...
	// т.к. мы ждём старый сторадж, даже если уже выполнился новый -
	// нужно установить timeout, иначе запущенную горутину не прибить?
	// ...
