Задача в том, чтобы найти k наиболее часто встречающихся слов в данном списке слов. Если два слова имеют одинаковую частоту, то слово с меньшим лексикографическим порядком должно быть предпочтительнее.
...
func (h WordHeap) Less(a, b int) bool {
  return h[a].Freq < h[b].Freq ||
    h[a].Freq == h[b].Freq && h[a].Word > h[b].Word
}
...
func topKFrequent(words []string, k int) []string {
  ...
  for i := range ln {
    result[ln-i-1] = heap.Pop(h).(WordFreq).Word  // !! нельзя извлекать через (*h)[ln-i-1].Word
  }
  return result
}
