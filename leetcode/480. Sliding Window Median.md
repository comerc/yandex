...
func medianSlidingWindow(nums []int, k int) []float64 {
	...
	window := nums[:k]
	sortedWindow := slices.Clone(window)
	slices.Sort(sortedWindow)
	mid := k / 2
	isEven := k%2 == 0
	// Функция для нахождения медианы в отсортированном списке
	getMedian := func() float64 {
		if isEven {
			return float64(sortedWindow[mid-1])/2 + float64(sortedWindow[mid])/2
		} else {
	...
	for _, v := range nums[k:] {
		...
		old, _ := slices.BinarySearch(sortedWindow, window[0])
		sortedWindow = slices.Delete(sortedWindow, old, old+1)
		// Добавляем новый элемент в сортированное окно:
		new, _ := slices.BinarySearch(sortedWindow, v)
		sortedWindow = slices.Insert(sortedWindow, new, v)
